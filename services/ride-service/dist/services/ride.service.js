...
let RideService = class RideService {
    ...
    async verifyZoneSequence(route) {
        const stops = await this.routeService.getStopsForRoute(route._id.toString());
        const zoneDistances = await Promise.all(stops.map(async (stop) => {
        ...
        if (route.startFromGIU) {
            if (zoneDistances[0].distance !== 0) {
                throw new common_1.BadRequestException('Route starting from GIU must have GIU as the first stop');
            }
            for (let i = 1; i < zoneDistances.length; i++) {
                if (zoneDistances[i].distance < zoneDistances[i - 1].distance) {
                    throw new common_1.BadRequestException('Route starting from GIU cannot go back to zones closer to GIU');
                }
            }
        }
        else {
            if (zoneDistances[zoneDistances.length - 1].distance !== 0) {
                throw new common_1.BadRequestException('Route ending at GIU must have GIU as the last stop');
            }
            for (let i = 1; i < zoneDistances.length; i++) {
                if (zoneDistances[i].distance > zoneDistances[i - 1].distance) {
                    throw new common_1.BadRequestException('Route ending at GIU must have zones getting progressively closer to GIU');
                }
            }
        }
    }
    async update(id, updateRideInput, userId) {
        ...
        if (ride.driverId !== userId) {
            throw new common_1.BadRequestException('You can only update your own rides');
        }
        ...
        if (updateRideInput.totalSeats !== undefined) {
            ...
            if (updateRideInput.totalSeats < currentBookings) {
                throw new common_1.BadRequestException(`Cannot decrease total seats below current bookings (${currentBookings})`);
            }
        ...
        if (updateRideInput.availableSeats !== undefined) {
            ...
            if (updateRideInput.availableSeats < 0) {
                throw new common_1.BadRequestException('Available seats cannot be negative');
            }
            if (updateRideInput.availableSeats > totalSeats - currentBookings) {
                throw new common_1.BadRequestException(`Available seats cannot exceed total seats minus current bookings (${totalSeats - currentBookings})`);
            }
        }
